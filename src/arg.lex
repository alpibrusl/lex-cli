...
# A flag definition — describes one --flag or -f entry in a CLI.
type FlagDef = {
  name        :: Str,
  short       :: Str,
  description :: Str,
...
type CliDef = {
  ...
  flags       :: List[FlagDef],
  positionals :: List[PositionalDef],
  subcommands :: List[SubcommandDef],
}
...
fn get_flag_bool(parsed :: ParsedArgs, name :: Str) -> Bool {
  list.fold(parsed.flags, false,
    fn (acc :: Bool, pair :: (Str, FlagValue)) -> Bool {
      match pair {
        (k, v) => if k == name {
          match v {
            FlagBool(b) => b,
...
fn get_flag_str(parsed :: ParsedArgs, name :: Str, default :: Str) -> Str {
  list.fold(parsed.flags, default,
    fn (acc :: Str, pair :: (Str, FlagValue)) -> Str {
      match pair {
        (k, v) => if k == name {
          match v {
            FlagStr(s)  => s,
