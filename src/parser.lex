...
fn parse_loop(
  ...
  argv :: List[Str],
  acc  :: arg.ParsedArgs
) -> Result[arg.ParsedArgs, arg.ParseError] {
  match list.nth(argv, 0) {
    None    => Ok(acc),
    Some(token) => {
      let rest := list.slice(argv, 1, list.len(argv))
        ...
        if str.starts_with(token, "-") and str.len(token) == 2 {
          ...
          match find_flag_by_short(cli.flags, short) {
            ...
            Some(fd) => {
              ...
              let acc2 := { subcommand: acc.subcommand, flags: new_flags,
                            positionals: acc.positionals, remaining: acc.remaining }
          ...
          if str.is_empty(acc.subcommand) and not list.is_empty(cli.subcommands) {
            ...
            match find_subcommand(cli.subcommands, token) {
              Some(_) => {
                let acc2 := { subcommand: token, flags: acc.flags,
                              positionals: acc.positionals, remaining: acc.remaining }
                parse_loop(cli, rest, acc2)
              },
            ...
            let acc2 := { subcommand: acc.subcommand, flags: acc.flags,
                          positionals: new_pos, remaining: acc.remaining }
            parse_loop(cli, rest, acc2)
          }
        }
      }
...
fn parse_long_flag(
  ...
  rest :: List[Str],
  acc  :: arg.ParsedArgs
) -> Result[LongFlagState, arg.ParseError] {
  # Check for `--name=value` form first.
  match find_eq(body, 0) {
    ...
    None => {
      # No `=` — either a bool flag or `--name value` (space-separated).
      match find_flag_by_name(cli.flags, body) {
        None => Err(arg.UnknownFlag(str.concat("--", body))),
        Some(fd) => {
          match fd.default {
            arg.FlagBool(_) => {
              # Bool flag: consume token only.
            ...
            arg.FlagStr(_) => {
              # String flag: consume next token as value.
              match list.nth(rest, 0) {
                None => {
                ...
                Some(value) => {
                  let rest2 := list.slice(rest, 1, list.len(rest))
...
fn find_eq(s :: Str, pos :: Int) -> Option[Int] {
  if pos >= str.len(s) { None }
  else {
    if str.slice(s, pos, pos + 1) == "=" { Some(pos) }
...
fn find_flag_by_name(flags :: List[arg.FlagDef], name :: Str) -> Option[arg.FlagDef] {
  list.fold(flags, None,
    fn (acc :: Option[arg.FlagDef], fd :: arg.FlagDef) -> Option[arg.FlagDef] {
      match acc {
        Some(_) => acc,
        None    => if fd.name == name { Some(fd) } else { None },
      }
    })
}
...
fn find_flag_by_short(flags :: List[arg.FlagDef], short :: Str) -> Option[arg.FlagDef] {
  list.fold(flags, None,
    fn (acc :: Option[arg.FlagDef], fd :: arg.FlagDef) -> Option[arg.FlagDef] {
      match acc {
        Some(_) => acc,
        None    => if fd.short == short { Some(fd) } else { None },
      }
    })
}
...
fn find_subcommand(
  subs :: List[arg.SubcommandDef],
  name :: Str
) -> Option[arg.SubcommandDef] {
  list.fold(subs, None,
    fn (acc :: Option[arg.SubcommandDef], sd :: arg.SubcommandDef) -> Option[arg.SubcommandDef] {
      match acc {
        Some(_) => acc,
        None    => if sd.name == name { Some(sd) } else { None },
      }
    })
}
