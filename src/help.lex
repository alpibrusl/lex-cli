...
fn render(cli :: arg.CliDef) -> Str {
  let header := str.concat(cli.name, str.concat(" v", str.concat(cli.version, str.concat(" — ", cli.description))))
...
fn render_subcommand(cli_name :: Str, sub :: arg.SubcommandDef) -> Str {
  let header := str.concat(cli_name, str.concat(" ", str.concat(sub.name, str.concat(" — ", sub.description))))
