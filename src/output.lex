...
fn exit_invalid_args() -> Int {
  2
}

fn exit_not_found() -> Int {
  3
}

fn exit_conflict() -> Int {
  5
}
...
# ---- Output mode -----------------------------------------------------
type OutputMode = OutputText | OutputJson
...
fn ok_json(command :: Str, data :: jv.Json) -> Str {
  let obj := JObj([("ok", JBool(true)), ("command", JStr(command)), ("data", data), ("meta", JObj([("duration_ms", JInt(0))]))])
  jv.stringify(obj)
}

# Build an error ACLI JSON envelope string.
fn err_json(command :: Str, message :: Str) -> Str {
  let obj := JObj([("ok", JBool(false)), ("command", JStr(command)), ("error", JStr(message)), ("meta", JObj([("duration_ms", JInt(0))]))])
  jv.stringify(obj)
}
...
fn print_ok(mode :: OutputMode, command :: Str, text :: Str, data :: jv.Json) -> [io] Nil {
  match mode {
...
fn print_err(mode :: OutputMode, command :: Str, text :: Str) -> [io] Nil {
  match mode {
    OutputText => io.print(text),
